...
const FTRiderSummaryQuery = 
    ...
    SELECT lifetime_salary, monthly_base_salary, monthly_orders, hours_worked
    FROM LifetimeSalary, MonthlySalary, MonthlyOrders, HoursWorked`;
...
riderSumRouter.get("/ftrider", async (req, res) => {
    const { rows } = await pgPool.query(FTRiderSummaryQuery, [req.cookies.uid]);
